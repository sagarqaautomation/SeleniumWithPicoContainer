...
public class WebTestBase {
	...
	public void start(WebTestContext webTestContext) {
		try {
		...
		webTestContext.getWebDriverManager().setWebDriver(driver);
	}

	public void teardown(WebTestContext webTestContext) {
		try {
			if (webTestContext.getWebDriverManager().getWebDriver() != null) {
				webTestContext.getWebDriverManager().getWebDriver().quit();
				webTestContext.setWebDriverManager(null);
				webTestContext.setWebPageObject(null);
			}
		} catch (Exception e) {
			webTestContext.setWebPageObject(null);
			webTestContext.setWebDriverManager(null);
